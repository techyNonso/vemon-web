    ...
    if request.method == "POST":
        ...
        if serializer.is_valid():
            ...
            message = {
                ...
                "email_subject": "Verify your email account",
            }
...
def resendValidationEmail(request):
    ...
    if request.method == "POST":
        ...
        if (
            ...
            and not Account.objects.get(email=email).is_active
        ):
            data = {}
...
def contactMessage(
    ...
    if request.method == "POST":
        ...
        if Util.send_email(message):
            ...
            return Response(
                {"message": "Email not sent"}, status=status.HTTP_502_BAD_GATEWAY
            )
    ...
    if request.method == "GET":
        ...
        try:
            ...
            return CustomRedirect(
                url + "/verify-email?verified=true&info=Email verified"
            )
            ...
            return CustomRedirect(
                url
                + "/verify-email?verified=false&info=The activation link has expired"
            )

            # return Response({'error':"activation link expired"},status=status.HTTP_400_BAD_REQUEST)
        except jwt.exceptions.DecodeError as identifier:
            return CustomRedirect(
                url + "/verify-email?verified=false&info=Invalid activation link"
            )
...
class RequestPasswordResetEmail(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        if Account.objects.filter(email=email).exists():
            ...
            return Response(
                {
                    "success": "A password reset link has been sent to the email you provided."
                },
    ...
    def get(self, request, uidb64, token):
        ...
        try:
            ...
            if not PasswordResetTokenGenerator().check_token(user, token):
                return CustomRedirect(
                    url
                    + "/reset-password?verified=false&info=This token is not valid, please request for a new one"
                )

            return CustomRedirect(
                url
                + "/reset-password?verified=true&info=Credentials valid&uidb64="
                + uidb64
                + "&token="
                + token
            )
            ...
            return CustomRedirect(
                url
                + "/reset-password?verified=false&info=Token is not valid, please request for a new one"
            )
...
class SetNewPasswordApiView(generics.GenericAPIView):
    ...
    def patch(self, request):
        ...
        serializer.is_valid(raise_exception=True)
        return Response(
            {"success": True, "message": "password reset successful. Proceed to login"},
            status=status.HTTP_200_OK,
        )
